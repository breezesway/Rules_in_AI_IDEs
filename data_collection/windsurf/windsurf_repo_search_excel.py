...
class GitHubSearch:
    ...
    def search_repositories_by_description(self, query: Union[str, List[str]], is_current_search: bool = True) -> None:
        ...
        def build_query_chunks(keywords: List[str]) -> List[str]:
            ...
            # 如果该关键词本身不包含 'windsurf'，则与 'windsurf' 共同作为必需条件
            chunks: List[str] = []
            for k in keywords:
                if not k or not k.strip():
                    continue
                token = quote_if_needed(k)
                if 'windsurf' in k.lower():
                    chunks.append(token)
                else:
                    # 默认词间即 AND 关系：token 与 windsurf 同时出现
                    chunks.append(f"{token} windsurf")
            return chunks
...
def main():
    ...
    # Use the same keyword set as in windsurf_repo_filter.py
    keywords = [
        #"windsurf",
