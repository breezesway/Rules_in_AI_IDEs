...
def check_qoder_folder(repo_url, github_token=None):
    """
    Check if repository contains .qoder folder
    """
    # Convert GitHub URL to API URL
    if 'github.com' not in repo_url:
        return False
    ...
    try:
        # Check for .qoder folder
        qoder_folder_url = f"https://api.github.com/repos/{owner}/{repo}/contents/.qoder"
        response = github_request(qoder_folder_url, headers=headers)
        if response is not None and response.status_code == 200:
            return True
        
        return False
    except Exception as e:
        print(f"Error checking .qoder folder for {repo_url}: {str(e)}")
        return False
...
def filter_csv_by_keywords():
    ...
    exclusion_keywords = [
        ...
        "a qoder", "重置", "无限试用", "curse", "3D qoder", "your qoder", 
        "custom qoder", "教程", "qoder navigation", "navigation", "qoder move", "学习", 
        ...
        "oracle", "store procedure", "mysql", "sql server", "qoder library", "Drawing", "draw", 
        "prompt", "collection", "hand gesture", "canvas", "click", "qoder array", "SQL", 
    ...
    # Discover input file: the only .xlsx that starts with 'qoder_all_repo'
    candidates = [
        os.path.join(script_dir, f)
        for f in os.listdir(script_dir)
        if f.startswith("qoder_all_repo") and f.endswith(".xlsx")
    ]
    if len(candidates) != 1:
        print("Error: Expected exactly one input file starting with 'qoder_all_repo' and ending with '.xlsx'.")
    ...
    print("\nFetching GitHub statistics and checking .qoder folder for repositories...")
    stats_data = []
    status_codes = []  # 用于存储每个仓库的状态码
    qoder_folder_data = []  # 用于存储qoder文件夹检查结果
    valid_indices = []  # 用于存储有效的索引
    
    for idx, url in enumerate(tqdm(filtered_df['url'])):
        # Check for .qoder folder first
        has_qoder_folder = check_qoder_folder(url, github_token)
        qoder_folder_data.append(has_qoder_folder)
        
        # Only process repositories that have .qoder folder
        if has_qoder_folder:
            ...
            # Skip repositories without .qoder folder
            print(f"\nSkipping {url} - no .qoder folder found")
        
        time.sleep(1)  # Rate limiting
    
    # Filter the dataframe to only include repositories with .qoder folder
    filtered_df = filtered_df.iloc[valid_indices].reset_index(drop=True)
    
    # 添加状态码列
    filtered_df['status_code'] = status_codes
    filtered_df['has_qoder_folder'] = [qoder_folder_data[i] for i in valid_indices]
    ...
    print(f"After qoder folder filtering: {len(filtered_df)}")
