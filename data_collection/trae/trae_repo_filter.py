...
def check_trae_folder(repo_url, github_token=None):
    """
    Check if repository contains .trae folder
    """
    # Convert GitHub URL to API URL
    if 'github.com' not in repo_url:
        return False
    ...
    try:
        # Check for .trae folder
        trae_folder_url = f"https://api.github.com/repos/{owner}/{repo}/contents/.trae"
        response = github_request(trae_folder_url, headers=headers)
        if response is not None and response.status_code == 200:
            return True
        
        return False
    except Exception as e:
        print(f"Error checking .trae folder for {repo_url}: {str(e)}")
        return False
...
def filter_csv_by_keywords():
    ...
    # Discover input file: the only .xlsx that starts with 'trae_all_repo'
    candidates = [
        os.path.join(script_dir, f)
        for f in os.listdir(script_dir)
        if f.startswith("trae_all_repo") and f.endswith(".xlsx")
    ]
    if len(candidates) != 1:
        print("Error: Expected exactly one input file starting with 'trae_all_repo' and ending with '.xlsx'.")
    ...
    print("\nFetching GitHub statistics and checking .trae folder for repositories...")
    stats_data = []
    status_codes = []  # 用于存储每个仓库的状态码
    trae_folder_data = []  # 用于存储trae文件夹检查结果
    valid_indices = []  # 用于存储有效的索引
    
    for idx, url in enumerate(tqdm(filtered_df['url'])):
        # Check for .trae folder first
        has_trae_folder = check_trae_folder(url, github_token)
        trae_folder_data.append(has_trae_folder)
        
        # Only process repositories that have .trae folder
        if has_trae_folder:
            ...
            # Skip repositories without .trae folder
            print(f"\nSkipping {url} - no .trae folder found")
        
        time.sleep(1)  # Rate limiting
    
    # Filter the dataframe to only include repositories with .trae folder
    filtered_df = filtered_df.iloc[valid_indices].reset_index(drop=True)
    
    # 添加状态码列
    filtered_df['status_code'] = status_codes
    filtered_df['has_trae_folder'] = [trae_folder_data[i] for i in valid_indices]
    ...
    print(f"After trae folder filtering: {len(filtered_df)}")
