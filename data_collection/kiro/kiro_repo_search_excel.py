...
class GitHubSearch:
    ...
    def _save_search_results(self, search_type: str, repos_dict: Dict[str, Dict], timestamp: str = None) -> None:
        ...
        excel_filename = os.path.join(self.output_dir, f'kiro_{search_type}_{timestamp}.xlsx')
    ...
    def search_repositories_by_description(self, query: Union[str, List[str]], is_current_search: bool = True) -> None:
        ...
        def build_query_chunks(keywords: List[str]) -> List[str]:
            ...
            # 如果该关键词本身不包含 'kiro'，则与 'kiro' 共同作为必需条件
            chunks: List[str] = []
            for k in keywords:
                ...
                if 'kiro' in k.lower():
                    ...
                    # 默认词间即 AND 关系：token 与 kiro 同时出现
                    chunks.append(f"{token} kiro")
            return chunks
    ...
    def save_results(self) -> None:
        ...
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        excel_filename = os.path.join(self.output_dir, f'kiro_all_repositories_{timestamp}.xlsx')
...
def main():
    ...
    print("Starting search for kiro-related repositories...")
    
    # Use the same keyword set as in kiro_repo_filter.py
    keywords = [
        #"kiro",
        "kiro IDE", "kiro AI",
