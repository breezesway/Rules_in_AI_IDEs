...
def check_cursor_files(repo_url, github_token=None):
    """
    Check if repository contains .cursorrules file or .cursor folder
    """
    # Convert GitHub URL to API URL
    if 'github.com' not in repo_url:
        return False
    ...
    try:
        ...
        cursorrules_url = f"https://api.github.com/repos/{owner}/{repo}/contents/.cursorrules"
        response = requests.get(cursorrules_url, headers=headers)
        has_cursorrules = response.status_code == 200
        
        # Check for .cursor folder
        cursor_folder_url = f"https://api.github.com/repos/{owner}/{repo}/contents/.cursor"
        response = requests.get(cursor_folder_url, headers=headers)
        has_cursor_folder = response.status_code == 200
        
        return has_cursorrules or has_cursor_folder
        
    except Exception as e:
        print(f"Error checking cursor files for {repo_url}: {str(e)}")
        return False
...
def filter_csv_by_keywords():
    ...
    # Discover input file: the only .xlsx that starts with 'cursor_all_repo'
    candidates = [
        os.path.join(script_dir, f)
        for f in os.listdir(script_dir)
        if f.startswith("cursor_all_repo") and f.endswith(".xlsx")
    ]
    if len(candidates) != 1:
        print("Error: Expected exactly one input file starting with 'cursor_all_repo' and ending with '.xlsx'.")
    ...
    print("\nFetching GitHub statistics and checking cursor files for repositories...")
    stats_data = []
    status_codes = []  # 用于存储每个仓库的状态码
    cursor_files_data = []  # 用于存储cursor文件检查结果
    valid_indices = []  # 用于存储有效的索引
    
    for idx, url in enumerate(tqdm(filtered_df['url'])):
        # Check for cursor files first
        has_cursor_files = check_cursor_files(url, github_token)
        cursor_files_data.append(has_cursor_files)
        
        # Only process repositories that have cursor files
        if has_cursor_files:
            ...
            # Skip repositories without cursor files
            print(f"\nSkipping {url} - no .cursorrules file or .cursor folder found")
        
        time.sleep(1)  # Rate limiting
    
    # Filter the dataframe to only include repositories with cursor files
    filtered_df = filtered_df.iloc[valid_indices].reset_index(drop=True)
    
    # 添加状态码列
    filtered_df['status_code'] = status_codes
    filtered_df['has_cursor_files'] = [cursor_files_data[i] for i in valid_indices]
    ...
    print(f"After cursor files filtering: {len(filtered_df)}")
