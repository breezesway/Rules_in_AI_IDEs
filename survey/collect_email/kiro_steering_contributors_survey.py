...
class KiroSteeringContributorsSurvey:
    ...
    def _check_kiro_steering_folder(self, owner: str, repo: str) -> bool:
        """Check if repository contains .kiro/steering folder."""
        kiro_steering_folder_url = f"{self.base_url}/repos/{owner}/{repo}/contents/.kiro/steering"
        data = self._make_request(kiro_steering_folder_url)
    ...
    def search_repositories(self, start_date: str = "2025-10-01", end_date: str = "2026-01-10"):
        """Search repositories with .kiro/steering folder in the specified date range."""
        print(f"Searching for repositories containing .kiro/steering folder")
        print(f"Date range: {start_date} to {end_date}")
        print("=" * 60)
        
        # Step 0: Get total count of matching repositories (first page only, for total_count)
        url = f"{self.base_url}/search/code"
        query = 'path:.kiro/steering'
        params = {
            'q': query,
        ...
        # Step 1: Fetch all code results containing .kiro/steering (up to 1000)
        print("\nStep 1: Fetching all code results containing .kiro/steering...")
        params = {
            'q': query,
            'per_page': 100
        }
        ...
        for idx, item in enumerate(all_code_items, 1):
            ...
            # Get commit history for .kiro/steering
            commits = self._get_file_commits(owner, repo, '.kiro/steering')
            if not commits:
        ...
        print(f"\nStep 3: Verifying each repository contains .kiro/steering folder...")
        # Step 3: Verify each repository contains .kiro/steering folder
        final_valid_repos = []
        for idx, repo_url in enumerate(valid_repos, 1):
            ...
            # Check if repository contains .kiro/steering folder
            if self._check_kiro_steering_folder(owner, repo):
    ...
    def process_repositories(self, repo_urls: List[str]):
        """Process repositories: extract contributors from .kiro/steering folder."""
        ...
        for idx, repo_url in enumerate(repo_urls, 1):
            ...
            # Confirm repository contains .kiro/steering folder
            if not self._check_kiro_steering_folder(owner, repo):
                print(f"  Skipped: No .kiro/steering folder found")
                continue
            
            print(f"  Found .kiro/steering folder, fetching commit history...")
            
            # Get commit history for the .kiro/steering folder
            commits = self._get_file_commits(owner, repo, ".kiro/steering")
            if not commits:
                print(f"  Warning: No commits found for .kiro/steering")
                continue
    ...
    def save_to_csv(self, filename: str = None):
        ...
        if filename is None:
            ...
            filename = os.path.join(self.output_dir, f'kiro_steering_contributors_{timestamp}.csv')
...
def main():
    ...
    # Initialize survey
    survey = KiroSteeringContributorsSurvey(token)
    ...
    print("=" * 60)
    print("Kiro Steering Contributors Survey")
    print("=" * 60)
    print(f"Date range: {start_date} to {end_date}")
    print("Criteria: Repository must contain .kiro/steering folder")
    print("=" * 60)
