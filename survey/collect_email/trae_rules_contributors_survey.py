...
class TraeRulesContributorsSurvey:
    ...
    def _check_trae_rules_folder(self, owner: str, repo: str) -> bool:
        """Check if repository contains .trae/rules folder."""
        trae_rules_folder_url = f"{self.base_url}/repos/{owner}/{repo}/contents/.trae/rules"
        data = self._make_request(trae_rules_folder_url)
    ...
    def search_repositories(self, start_date: str = "2025-10-01", end_date: str = "2026-01-10"):
        """Search repositories with .trae/rules folder in the specified date range."""
        print(f"Searching for repositories containing .trae/rules folder")
        print(f"Date range: {start_date} to {end_date}")
        print("=" * 60)
        
        # Step 0: Get total count of matching repositories (first page only, for total_count)
        url = f"{self.base_url}/search/code"
        query = 'path:.trae/rules'
        params = {
            'q': query,
        ...
        # Step 1: Fetch all code results containing .trae/rules (up to 1000)
        print("\nStep 1: Fetching all code results containing .trae/rules...")
        params = {
            'q': query,
            'per_page': 100
        }
        ...
        for idx, item in enumerate(all_code_items, 1):
            ...
            # Get commit history for .trae/rules
            commits = self._get_file_commits(owner, repo, '.trae/rules')
            if not commits:
        ...
        print(f"\nStep 3: Verifying each repository contains .trae/rules folder...")
        # Step 3: Verify each repository contains .trae/rules folder
        final_valid_repos = []
        for idx, repo_url in enumerate(valid_repos, 1):
            ...
            # Check if repository contains .trae/rules folder
            if self._check_trae_rules_folder(owner, repo):
    ...
    def process_repositories(self, repo_urls: List[str]):
        """Process repositories: extract contributors from .trae/rules folder."""
        ...
        for idx, repo_url in enumerate(repo_urls, 1):
            ...
            # Confirm repository contains .trae/rules folder
            if not self._check_trae_rules_folder(owner, repo):
                print(f"  Skipped: No .trae/rules folder found")
                continue
            
            print(f"  Found .trae/rules folder, fetching commit history...")
            
            # Get commit history for the .trae/rules folder
            commits = self._get_file_commits(owner, repo, ".trae/rules")
            if not commits:
                print(f"  Warning: No commits found for .trae/rules")
                continue
    ...
    def save_to_csv(self, filename: str = None):
        ...
        if filename is None:
            ...
            filename = os.path.join(self.output_dir, f'trae_rules_contributors_{timestamp}.csv')
...
def main():
    ...
    # Initialize survey
    survey = TraeRulesContributorsSurvey(token)
    ...
    print("=" * 60)
    print("Trae Rules Contributors Survey")
    print("=" * 60)
    print(f"Date range: {start_date} to {end_date}")
    print("Criteria: Repository must contain .trae/rules folder")
    print("=" * 60)
