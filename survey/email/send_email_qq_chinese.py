"""
Batch send Chinese emails via QQ/Foxmail
"""
...
# ========== File paths ==========
TEMPLATE_FILE = os.path.join(os.path.dirname(__file__), "invite_email_chinese.txt")
...
def create_email_html(subject, body, name, project_name):
    ...
    # Replace [ Name] with bold name (note space in Chinese template)
...
def send_email_qq(sender_email, sender_password, recipient_email, subject, html_body, max_retries=2):
    ...
    for attempt in range(max_retries):
        ...
        try:
            ...
            # Connect to QQ SMTP server with 30-second timeout
            server = smtplib.SMTP_SSL('smtp.qq.com', 465, timeout=30)  # QQ email uses SSL on port 465
            ...
            if attempt < max_retries - 1:
                wait_time = (attempt + 1) * 5  # Wait 5s, 10s before retry
                print(f"  Connection failed, retrying in {wait_time}s (attempt {attempt + 1}/{max_retries})...")
                time.sleep(wait_time)
            else:
                print(f"  Send failed ({recipient_email}): SMTP error - {str(e)}")
                return False
            ...
            if attempt < max_retries - 1:
                ...
                print(f"  Connection timed out, retrying in {wait_time}s (attempt {attempt + 1}/{max_retries})...")
                time.sleep(wait_time)
            else:
                print(f"  Send failed ({recipient_email}): {str(e)}")
                return False
    return False
...
def main():
    ...
    for row_index, row in enumerate(all_rows):
        ...
        # Only process Chinese users (is_chinese_user is TRUE)
        is_chinese_user = get_column_value(row, 'is_chinese_user').strip().upper()
        if is_chinese_user != 'TRUE':
            continue
