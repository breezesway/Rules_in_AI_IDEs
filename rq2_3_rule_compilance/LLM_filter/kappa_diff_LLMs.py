"""
Compute Kappa for multiple filter_results files
- Fleiss' Kappa: agreement among multiple raters (3 files)
- Cohen's Kappa: pairwise Cohen's Kappa
"""
...
def load_json_file(file_path: Path) -> List[Dict[Any, Any]]:
    ...
    try:
        ...
        print(f"Error: cannot parse JSON file {file_path}: {e}")
        return []
...
def calculate_fleiss_kappa(data: np.ndarray) -> Tuple[float, Dict[str, Any]]:
    ...
    Args:
        data: numpy array of shape (n_items, n_raters)
              each row is an item, each column a rater's rating
    
    Returns:
        (kappa, stats): Kappa coefficient and statistics
    """
    ...
    # P[i][j] = count of category j ratings for item i
    ...
    # Pi: rater agreement for item i
    Pi = np.zeros(n_items)
    for i in range(n_items):
        sum_squared = np.sum(P[i, :] ** 2)
        Pi[i] = (sum_squared - n_raters) / (n_raters * (n_raters - 1))
    
    # Pbar: mean agreement across items
    Pbar = np.mean(Pi)
    
    # Pj: overall proportion per category
...
def calculate_cohen_kappa(y1: List[Any], y2: List[Any]) -> Tuple[float, Dict[str, Any]]:
    """Compute Cohen's Kappa with detailed statistics"""
    if len(y1) != len(y2):
...
def main():
    ...
    print("=" * 80)
    print("Computing Kappa for multiple filter_results files")
    print("=" * 80)
    ...
    if len(all_data) < 2:
        print("\nError: need at least 2 valid files to compute Kappa")
        return
    
    # Build rule_id-keyed dicts
    data_dicts = {}
    for filename, data in all_data.items():
        ...
        for item in data:
            if 'rule_id' in item:
                ...
                can_detect = item.get('can_detect', None)
                if can_detect is not None:
                    data_dict[rule_id] = can_detect
        data_dicts[filename] = data_dict
        print(f"  {filename}: extracted {len(data_dict)} valid can_detect values")
    
    # Common rule_ids across files
    if len(data_dicts) > 0:
    ...
    if len(common_rule_ids) == 0:
        print("Error: no common rule_ids; cannot compute Kappa")
        return
    
    # Extract can_detect per file (sorted by rule_id)
    ...
    if len(all_data) >= 3:
        ...
        data_array = np.zeros((n_items, n_raters), dtype=object)
        ...
        print(f"Only {len(all_data)} files present; skipping Fleiss' Kappa")
        print("=" * 80)
    ...
    for (i, filename1), (j, filename2) in combinations(enumerate(filenames_list), 2):
        ...
        unique_labels = sorted(set(values1 + values2))
        print("    File1 \\ File2", end="")
        for label2 in unique_labels:
            print(f"  {label2:>8}", end="")
        print()
        for label1 in unique_labels:
    ...
    if fleiss_kappa is not None:
        ...
        print(f"Fleiss' Kappa: not computed (needs 3+ files)")
    ...
    if pairwise_results:
        ...
        for result in pairwise_results:
            ...
            kappa_matrix[file2][file1] = kappa  # symmetric
        
        # All filenames for matrix rows/columns
        all_filenames = sorted(filenames_list)
        
        # Column width from shortened filenames
        short_names = [f.replace("filter_results_", "").replace(".json", "") for f in all_filenames]
        ...
        for filename in all_filenames:
            ...
            short_name = filename.replace("filter_results_", "").replace(".json", "")
        ...
        for filename1 in all_filenames:
            short_name1 = filename1.replace("filter_results_", "").replace(".json", "")
            print(f"{short_name1:>{col_width}}", end="  ")
            for filename2 in all_filenames:
                if filename1 == filename2:
                    # Diagonal shows 1.0000
