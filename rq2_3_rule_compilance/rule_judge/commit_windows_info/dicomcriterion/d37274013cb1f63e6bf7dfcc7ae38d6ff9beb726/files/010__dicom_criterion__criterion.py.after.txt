...
class Criterion:
    ...
    def __init__(self, expression: str, registry: Optional[FunctionRegistry] = None) -> None:
        ...
        try:
            self._parsed_expression = self._algebra.parse(boolean_expression, simplify=False)
        except Exception as e:
            raise ExpressionParseError(
                self._expression,
                f"Failed to parse boolean expression: {str(e)}"
            ) from e
        ...
        try:
            ...
            for symbol in expression_symbols:
                ...
                if symbol_key in self._symbol_mapping:
                    ...
                    raise ValueError(f"Symbol {symbol_key} not found in symbol mapping")
            ...
            return simplified_result == self._algebra.TRUE
        ...
        for match in matches:
            try:
                ...
                raise SymbolParseError(
                    match,
                    f"Invalid DICOM symbol found in expression: '{expression}'"
                )
        
        if not dicom_symbols:
            raise ExpressionParseError(
                expression,
                "No valid DICOM symbols found. Expected format: 'attribute.function(args)'"
            )
